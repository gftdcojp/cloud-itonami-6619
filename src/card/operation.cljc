(ns card.operation
  "OperationActor -- one card-processing operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Card Advisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Card Settlement Governor (:govern) and
  ...
  One graph run = one card-processing operation (intake -> advise ->
  ...
  decision to a human operator (the processor officer). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:settlement/finalize`/`:chargeback/release` ALWAYS reach this node
  when the governor is clean -- see `card.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [card.cardadvisor :as cardadvisor]
...
(defn build
  ...
  `card.store/Store`).
  opts:
    :advisor      -- a `card.cardadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (cardadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Card Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (cardadvisor/-advise advisor store request)]
            {:proposal p :audit [(cardadvisor/trace request p)]})))

      ;; Card Settlement Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
